...
for _, s in pairs(game.surfaces) do
  ...
  for _, hidden in pairs(hiddens) do
    local g = storage.unum_to_g[hidden.unit_number]
    if g then
      if not (g.spotter 
              and g.spotter.valid 
              and g.spotter.unit_number == hidden.unit_number) then
        storage.unum_to_g[hidden.unit_number] = nil
      end
    end

    if not storage.unum_to_g[hidden.unit_number] then
      hidden.destroy()
    end
  end
end

for unum, g in pairs(storage.unum_to_g) do
  if not g.light.valid then
    storage.unum_to_g[unum] = nil
  else
